...
export default function HowItWorksSection() {
  ...
  return (
    <section id="ablauf" className="py-24 px-6 md:px-12" style={{ background: '#243558' }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-20">
          <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full bg-yellow-600/10 border border-yellow-600/30">
            <span className="text-yellow-500 text-xs tracking-widest uppercase">Ablauf</span>
          </div>
          <h2 className="font-serif text-5xl md:text-6xl font-bold text-white">So einfach funktioniert&apos;s</h2>
        </div>

        {/* Timeline */}
        <div ref={containerRef} className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
          {/* Connection Line */}
          <div className="hidden md:block absolute top-8 left-8 right-8 h-1 bg-gradient-to-r from-yellow-500/20 via-yellow-500/40 to-yellow-500/20 -z-10" />

          {steps.map((step, idx) => (
            <div key={idx} className="timeline-step reveal">
              {/* Dot */}
              <div className="w-16 h-16 border-4 border-yellow-500 rounded-full bg-yellow-500/10 flex items-center justify-center font-serif text-2xl font-bold text-yellow-500 mb-8 shadow-lg">
                {step.number}
              </div>

              {/* Content */}
              <h3 className="font-serif text-xl font-bold text-white mb-4">{step.title}</h3>
